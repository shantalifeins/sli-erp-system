...
### 🔒 Git Repository Isolation & Automated Push Rules
...
- **Automated Credential Scope**: To prevent background command subshells from blocking on interactive Windows Credential Manager popups, local credentials are bound to `git config --local credential.username shantalifeins`.
...
- **Depreciation Reports (`/asset-depr-reports`)**: Standalone depreciation reporting module with two tabs:
  - **Summary Report** (`GET /api/assets/reports/depr-summary?startMonth=YYYY-MM&endMonth=YYYY-MM`): Category-wise multi-level Cost vs. Depreciation table with columns: Opening Balance, Addition, Disposal, Closing Balance per cost and depreciation group, Depreciation Rate %, and Written-Down Value (WDV). Computes opening depreciation balance by tracing the last Posted schedule row before `startDate` per asset; falls back to `accumulatedDepreciation − in-period charges`. Groups by `asset_categories`.
  - **Detailed Report** (`GET /api/assets/reports/depr-detailed?startMonth=YYYY-MM&endMonth=YYYY-MM`): Line-by-line transaction view grouped by category. Emits one synthetic **Opening balance** row per category (cost of assets acquired before the period, accumulated dep before the period) followed by individual `Depreciation Charge` rows from `asset_depreciation_schedule` (status=`Posted`). Columns: Date, FS Category, Description, Opening, Addition, Acc Dep, Net Cost, Year End, Rate %, Depreciation.
  - Both endpoints accept optional `categoryId` and `branchId` query filters, return period metadata, totals row, and are multi-tenant guarded via `resolveTenantId`. Frontend supports CSV export, Print, and live filter bar with month-range pickers.
  - RBAC: `'Depreciation Reports'` action `canView` registered in `hierarchy` in `Admin.tsx`.
